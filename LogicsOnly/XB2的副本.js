...
var PositionOfPreviousStep = 0;
var PositionOfCurrentStep = 0;
var SizeOfPreviousStep = 1;
var SizeOfCurrentStep = 0;
var StepAmount = 1;
var Score = 0;
var StepQueue = new Array();
var StepLoopCount = 0;
AR.onload = function() {
...
AR.ontouch = function(x, y, state) {
	if (CurStepIndex >= StepNodeNameArray.length) return;
	if (state == 0 && !PressBegin) {
		OnTouchPress();
	}

	if (state == 1 && PressBegin) {
		OnTouchRelease();
	}
}
...
function SimulationScreenTouch() {
	// if (CurStepIndex >= StepNodeNameArray.length)CurStepIndex=1;
...
function OnTouchPress() {
	...
	//Calculater next pos
	var tmp_CurrentStepYPosition = AR.get_position(tmp_NodeName).y;
	...
	AR.set_visible(StepNodeNameArray[tmp_Index], true);
	AR.play(StepNodeNameArray[tmp_Index] + "Scale", 1);

	AR.modulate_color(StepNodeNameArray[tmp_Index], Math.random(), Math.random(), Math.random(), 1, 0);

	StepQueue.push(StepNodeNameArray[tmp_Index]);
}
...
function OnTouchRelease() {
	...
	//Stop animation
	AR.stop(StepNodeNameArray[tmp_Index] + "Scale");
	...
	SizeOfCurrentStep = AR.get_scale(StepNodeNameArray[tmp_Index]).x;

	//Color of hint
	AR.translate("Guangquan_001", 0, HeightofeachLightstep, 0);
	...
	if (CurStepIndex > 3) {
		...
		AR.set_visible(StepNodeNameArray[StepLoopCount % CurStepIndex], false);
		AR.scale(StepNodeNameArray[StepLoopCount % CurStepIndex], 0, 0, 0);
		StepLoopCount++;
	}
}
