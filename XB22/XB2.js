...
var CurStepIndex = 1;
var StepLoopCount = 0;
var SizeOfCurrentStep = 0;
var SizeOfPreviousStep = 1;
var HeightOfEachStep = 4;
var MaximumCountOfSteps = 6;
var HeightofeachLightstep = 4;
...
Queue = {
	Enqueue: function(_element) {
		QueueArray.push(_element);
	},
	DeQueue: function() {
		var tmp_value = QueueArray[0];
	...
	SeachInQueue: function(_index) {
		return QueueArray[_index];
	},
	Clear: function() {
...
var CurrentStepHintNodeName = null;

//Step material gradenit
var StartColor = {
	r: 161 / 255,
	g: 140 / 255,
	b: 209 / 255,
...
/**
 * [SimulationScreenTouch description]Simulation touch
 */
...
/**
 * [OnTouchPress description]Touch down
 */
function OnTouchPress() {

	//Setup step color and position
	var tmp_Index = CurStepIndex % StepGroupsNodeName.length;
	var tmp_NodeName = StepGroupsNodeName[tmp_Index];

	//Calculater next pos
	var tmp_CurrentStepYPosition = AR.get_position(tmp_NodeName).y - WorldOrigin.y;
	var tmp_NextStepPos = HeightOfEachStep * CurStepIndex - tmp_CurrentStepYPosition - StepLoopCount * HeightOfEachStep;
	AR.translate(tmp_NodeName, 0, tmp_NextStepPos, 0);
	AR.translate(StepHintsNodeName[9], 0, HeightofeachLightstep, 0);
...
function OnTouchRelease() {
	...
	var tmp_CurrentStepBlockNodeName = StepBlocksNodeName[tmp_Index];
	CurrentStepHintNodeName = StepHintsNodeName[9];
	...
	//Q:Why do we need queueArray.length - 2?
	//A:Need to get the size of pre-step in the current queue.
	SizeOfPreviousStep = AR.get_scale(Queue.SeachInQueue(QueueArray.length - 2)).x; //(StepGroupsNodeName[tmp_Index - 1]).x;
	SizeOfCurrentStep = AR.get_scale(tmp_CurrentStepGroupNodeName).x;

	//Color of hint
	AR.set_visible(CurrentStepHintNodeName, true);
	AR.play(CurrentStepHintNodeName + "Scale", 1);
	...
	var AlphaTimer = AR.setInterval(function() {
		if (HintColor.a <= 0) {
			AR.scale(CurrentStepHintNodeName, 0, 0, 0);
			AR.clearInterval(AlphaTimer);
		}
	...
	//Loop
	if (CurStepIndex > MaximumCountOfSteps) {
		var tmp_CurrentStepGroupHeight = AR.get_position("Group_block").y;
		AR.translate("Group_block", 0, -(HeightOfEachStep), 0);
		AR.set_visible(Queue.PopQueue(), false);
		AR.scale(Queue.DeQueue(), 0, 0, 0);
		StepLoopCount++;
	}
}
...
function ReStart() {
	...
	for (var e in StepGroupsNodeName) {
		...
		if (e > 0) {
			...
			AR.set_visible(StepGroupsNodeName[0], true)
		AR.play(StepGroupsNodeName[0] + "RestToOne", 1);
	}
	AR.play("Guangquan_0010Rest", 1);
...
/**
 * [MakingAnimation description]Create the step scaling animation
 */
...
Lerp = function(_valueFrom, _valueTo, _time) {
	return _valueFrom * (1 - _time) + _valueTo * _time;
}


function ColorGradenit(_nodeName) {
	var tmp_Time = Math.sin(CurStepIndex * 0.25);
	var tmp_CurrentTime = 0.0001;
	var tmp_ColorValue = {
		r: 0,
		g: 0,
		b: 0,
	...
	var tmp_ColorGradenit = AR.setInterval(function() {
		if (tmp_Time < 0.33) {
			var tmp_CurProgress = tmp_Time / 0.33;
			tmp_ColorValue.r = Lerp(StartColor.r, TwoFourthColor.r, tmp_CurProgress);
			tmp_ColorValue.g = Lerp(StartColor.g, TwoFourthColor.g, tmp_CurProgress);
			tmp_ColorValue.b = Lerp(StartColor.b, TwoFourthColor.b, tmp_CurProgress);
			if (tmp_CurProgress >= tmp_Time / 0.33) {
				AR.clearInterval(tmp_ColorGradenit);
			}
		} else if (tmp_Time < 0.66) {
			var tmp_CurProgress = (tmp_Time - 0.33) / 0.33;
			tmp_ColorValue.r = Lerp(TwoFourthColor.r, ThreeFourthColor.r, tmp_CurProgress);
			tmp_ColorValue.g = Lerp(TwoFourthColor.g, ThreeFourthColor.g, tmp_CurProgress);
			tmp_ColorValue.b = Lerp(TwoFourthColor.b, ThreeFourthColor.b, tmp_CurProgress);
			if (tmp_CurProgress >= (tmp_Time - 0.33) / 0.33) {
				AR.clearInterval(tmp_ColorGradenit);
			}
		} else {
			var tmp_CurProgress = (tmp_Time - 0.66) / 0.66;
			tmp_ColorValue.r = Lerp(ThreeFourthColor.r, EndColor.r, tmp_CurProgress);
			tmp_ColorValue.g = Lerp(ThreeFourthColor.g, EndColor.g, tmp_CurProgress);
			tmp_ColorValue.b = Lerp(ThreeFourthColor.b, EndColor.b, tmp_CurProgress);
			if (tmp_CurProgress >= (tmp_Time - 0.66) / 0.66) {
				AR.clearInterval(tmp_ColorGradenit);
			}
		}

		AR.modulate_color(_nodeName, tmp_ColorValue.r, tmp_ColorValue.g, tmp_ColorValue.b, tmp_ColorValue.a, 0);
	}, 20);
...
UI = {
	RefreshScore: function(_score) {
		var G = Score % 10;
		var T = Score / 10 % 10;

		AR.set_texture("UI1_shuzi_01", "bundle/number/" + Math.floor(G) + ".png");
		AR.set_texture("UI1_shuzi_02", "bundle/number/" + Math.floor(T) + ".png");
	},
	OnGameOver: function() {
		...
		AR.set_texture("UI2_shuzi_01", "bundle/number/" + Math.floor(T) + ".png");
		AR.set_texture("UI2_shuzi_02", "bundle/number/" + Math.floor(G) + ".png");
	},
